...
if __name__ == "__main__":
    ...
    with open(f"data/bm/orders/{date}/{date}.json", "r") as f:
        orders = json.load(f)
        insights = aggregateAndGetInsightsBackMarket(orders)
    ...
    # print("------------------------------------------------------------------------")
    # start = time.time()
    # with open("data/rf.json", "w") as f:
    #     f.write(json.dumps(InfoSource.mapEnumToGetParams()[InfoSource.Refurbed](), indent=3))
    # end = time.time()
    # total_time = end - start
    # print("------------RF Stats: It took {} seconds to make {} API calls------------".format(total_time, 1))
