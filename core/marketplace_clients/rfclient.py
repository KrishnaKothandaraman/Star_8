    ...
    def generateItemsBodyForSWDCreateOrderRequest(orderItems: List[dict], swdModelNames: List[str]) -> List[dict]:
        ...
        for swd_model_name, order_item in list(zip(swdModelNames, orderItems)):
            ...
            if "eus" in swd_model_name and any(
                    substr.lower() in swd_model_name for substr in ("iPad 9", "iPad 10")):
                adapter_item = [{
                    "skuType": "reference",
                    "sku": "002820",
                    "amount": quantity,
            ...
            elif "eus" in swd_model_name and any(substr.lower() in swd_model_name for substr in ("iPad 7", "iPad 6", "iPad 8")):
                adapter_item = [{
                    "skuType": "reference",
                    "sku": "002479",
                    "amount": quantity,
